...
import { EngineMatchDirector } from '../../controller/EngineMatchDirector';
import {
  DEFAULT_START_FEN,
  ENGINE_DISPLAY_NAME,
...
export class LocalController {
  ...
  /** CASO CRUZADO — two on-device Lughnasadh processes (white/black slots) */
  async startMatch(config: MatchConfig = {}): Promise<void> {
    await this.cancelAnalysis('cancelled');
    // Keep analysis adapter idle; match spawns its own white/black engines.
  ...
  private ensureMatch(): EngineMatchDirector {
    ...
    this.match = new EngineMatchDirector(
      (side, handlers) => new LocalLughnasadhAdapter(handlers, side === 'w' ? 'white' : 'black'),
      {
