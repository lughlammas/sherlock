#!/usr/bin/env bash
# Build Sherlock-0.1.1-s21.apk (arm64-v8a, signed) on the box.
set -euo pipefail
...
cp -f "$OUT" "$ROOT/Sherlock-0.1.1-s21.apk"
cp -f "$OUT" /workspace/Sherlock-0.1.1-s21.apk 2>/dev/null || true
echo "APK: $ROOT/Sherlock-0.1.1-s21.apk"
ls -la "$ROOT/Sherlock-0.1.1-s21.apk"
sha256sum "$ROOT/Sherlock-0.1.1-s21.apk"
